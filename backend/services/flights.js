...
amadeusFlightsRouter.post("/book", isAuthenticated, async (req, res) => {
    ...
    const { flightOffer, airlines, amountFromWallet } = req.body;
    ...
    try {
        ...
        if (tourist) {
            ...
            bookingResponse.data.createdAt = new Date();
        ...
        res.cookie("balance", tourist.wallet).status(200).send({ bookingResponse });
    } catch (error) {
        console.log(error);
        res.status(400).send({ error: error.message });
    }
