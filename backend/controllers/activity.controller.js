...
export const getActivityById = async (req, res) => {
	try {
		...
		if (activity) {
			const { toBeNotifiedTourists, createdAt, updatedAt, __v, ...others } = activity._doc;
...
export const getUpcomingActivities = async (req, res) => {
	try {
		...
		if (price) {
			...
			activities = activities.filter((activity) => {
				const activityPrice = activity.price * (1 - activity.specialDiscount / 100);
		...
		if (sortBy) {
			if (sortBy === "priceAsc") {
				activities.sort((a, b) => {
					const specialDiscountPriceA = (a.price * (100 - a.specialDiscount)) / 100;
					const specialDiscountPriceB = (b.price * (100 - b.specialDiscount)) / 100;
