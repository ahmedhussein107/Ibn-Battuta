...
export const deleteTourGuide = async (req, res) => {
    try {
        ...
        if (tourGuide) {
            // find all upcoming itineraries by this tour guide
            const upcomingItineraries = await Itinerary.find({
                tourguideID: req.params.id,
                "activities.endTime": { $gt: new Date() },
            }).distinct("_id");

            // Then check if any of these itineraries have bookings
            ...
            if (!hasUpcoming) {
                ...
                res.status(400).json({
                    message:
                        "Tour guide has created upcoming itineraries that already have bookings",
                });
            }
        } else {
            res.status(404).json({ e: "TourGuide not found" });
