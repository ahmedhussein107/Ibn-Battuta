...
export const changeAdminPassword = async (req, res) => {
	...
	try {
		...
		return res.status(400).json({ message: "An error occurred while changing the password" });
	}
};
...
export const getUsers = async (req, res) => {
	try {
		...
		let result = [];
		const isAccepted = req.query.isAccepted == null || req.query.isAccepted === "true";
		...
		for (const [role, model] of Object.entries(models)) {
			...
			if (!isAccepted && (role === "tourist" || role === "admin" || role === "governor"))
				continue;
