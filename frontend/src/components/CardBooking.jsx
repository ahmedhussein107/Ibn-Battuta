...
const CardBooking = ({ booking, width, height, fontSize = "1.5rem", setError }) => {
	...
	const [canRateTourGuide, setCanRateTourGuide] = useState(booking.ratingTourGuideID == null);
	...
	const date =
		booking.bookingType == "Itinerary" ? booking.typeId?.startDate : booking.typeId?.startDate;
	...
	const handleCancel = async () => {
		try {
			const response = await axiosInstance.delete(`/booking/deleteBooking/${booking._id}`);
					...
					<CustomButton
						...
						handleClick={() => {
							...
							if (type == "Itinerary" || type == "Activity") {
								navigate(
									`/${type.toLocaleLowerCase()}-details/${booking.typeId._id}`,
									{
										state: {
