		...
		<div className="complaint-card">
			...
			<div className="complaint-header">
				...
				<div className="status-and-actions">
					<span className={`complaint-status ${status}`}>{status.toUpperCase()}</span>
					{userType === "Admin" && isExpanded && (
						<div className={`change-status-${status}`} onClick={handleUpdateStatus}>
							{status === "pending" && (
								<DoneSharpIcon
									sx={{
			...
			<div className="complaint-footer">
				{!isExpanded ? (
					...
					<div className="reply-div" onClick={() => props?.onReply(null)}>
						<ReplySharpIcon sx={{ verticalAlign: "middle", marginRight: "5px" }} />
						reply
					</div>
				)}
			</div>
