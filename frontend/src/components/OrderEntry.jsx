...
import deletedProductImage from "../assets/images/out-of-stock.png";
import PopUp from "./PopUpsGeneric/PopUp";
import axiosInstance from "../api/axiosInstance";

const OrderEntry = ({ orderID, product, price, count, ratingID, formatPrice, orderStatus }) => {
	const name = product?.name || "Deleted Product";
	const description = product?.description || "No description available";
	const pictures = product?.pictures || [deletedProductImage];
	...
	const handleSubmit = async (event, newValue) => {
		try {
			...
			if (response.status === 201) {
				...
				const newOrder = await axiosInstance.patch(`/order/addRating/${orderID}`, {
	...
	return (
		<>
			...
			<div style={cartItemStyle}>
                ...
                ></div> */}
				<hr style={{ width: "98%", margin: "2vh", border: "1px solid #D1D5DB" }} />
			</div>
		</>
	);
};
