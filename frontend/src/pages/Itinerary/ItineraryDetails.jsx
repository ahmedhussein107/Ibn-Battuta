...
const ItineraryDetails = () => {
	...
	useEffect(() => {
		const fetchFreeSpots = async () => {
			if (!itinerary) return;
			try {
				const response = await axiosInstance.get(`itinerary/getFreeSpots/${itinerary._id}`);
			...
			try {
				const activitiesData = await Promise.all(
					itinerary.activities.map(async (activityObj) => {
						...
						const durationMs =
							new Date(activityObj.endTime) - new Date(activityObj.startTime);
				...
				const photosData = activitiesData
					.filter(
						(activity) =>
							activity.activityType === "Activity" && activity.activityData.pictures
					)
			...
			<div style={{ display: "flex", gap: "1rem" }}>
				...
				{!hasBookingID && (
					<Button
						...
						handleClick={() => setIsCheckoutPopupOpen(true)} // Set popup to open
						customStyle={{ marginBottom: "3vh" }}
					/>
				)}
			</div>
